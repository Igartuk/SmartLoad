...
namespace SmartLoad.Application.Packing.Commands
{
    public class CalculateLoadPlanHandler : IRequestHandler<CalculateLoadPlanCommand, LoadPlanResponse>
    {
        ...
        public CalculateLoadPlanHandler(IPackingStrategy packingStrategy, ILoadPlanRepository repository, UrlShortenerService urlShortener)
        {
        ...
        public async Task<LoadPlanResponse> Handle(CalculateLoadPlanCommand request, CancellationToken cancellationToken)
        {
            ...
            // Generate short URL and set it on the load plan
            var shortUrl = _urlShortener.GenerateUrlFromGuid(loadPlan.Id);
            loadPlan.SetUrl(shortUrl);
